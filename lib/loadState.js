...
var loadState = {
  ...
  createRFn: function createRFn(field) {
    return this.createFn(function (change) {
      ...
      this.setState(function (prevState) {
        var loading = prevState[field]; // 如果不存在 或 类型是布尔型 则认为是初始值 0 用于刚开始就显示加载状态true的情况

        if (!loading || _isTypeOf["default"]["boolean"](loading)) {
          loading = 0;
        }
  ...
  createVFn: function createVFn(field) {
    return this.createFn(function (change) {
      // 修改组件props中指定属性
      var loading = this[field]; // 如果不存在 或 类型是布尔型 则认为是初始值 0 用于刚开始就显示加载状态true的情况
